...
| `NavigationBar` | `tabs: NavTabItem<T>[]`, `active: T`, `onChange: (tab: T) => void` | Generic over the caller's own tab-key union `T extends string` (typically a route-name type). `NavTabItem<T> = { key: T, label: string, icon: LucideIcon }`. No built-in tab set — count, labels and icons are entirely caller-defined; pass 2 or more. |
